...
struct CgEdgeWithCalls {
  ...
  bool operator<(const CgEdgeWithCalls &other) const {
    return std::tie(calls, child, parent) < std::tie(other.calls, other.child, other.parent);
  }

  friend bool operator==(const CgEdgeWithCalls &lhs, const CgEdgeWithCalls &rhs) {
    return std::tie(lhs.calls, lhs.child, lhs.parent) == std::tie(rhs.calls, rhs.child, rhs.parent);
  }
...
struct MoreCallsOnEdge {
  bool operator()(const CgEdgeWithCalls &lhs, const CgEdgeWithCalls &rhs) { return lhs.calls < rhs.calls; }
};
