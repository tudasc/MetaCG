...
int DomCommand::run(const std::vector<char*>& args) { 
    ...
    try {
        ...
        if (!result.count("node") || !result.count("entry")) {
        ...
        std::string refName = result["entry"].as<std::string>();

        auto* targetNodePtr = findNode(cg.get(), nodeName, "Target node");
        auto* refNodePtr = findNode(cg.get(), refName, "Entry node");
        ...
        std::unordered_map<const metacg::CgNode*, metacg::analysis::DomData<metacg::CgNode>> dom =
            metacg::analysis::computeDoms<
            metacg::CgNode,
            metacg::Callgraph,
            metacg::analysis::TraverseDir::Forward>(*cg, *refNodePtr);
