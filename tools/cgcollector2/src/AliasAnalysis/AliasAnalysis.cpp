...
void ASTInformationExtractor::merge(EquivClassesIterator E1, EquivClassesIterator E2) {
  const auto Ret = mergeRecurisve(Data, E1, E2);
...
void ASTInformationExtractor::mergeFunctionCall(const StringType& CalledObj, CallInfoConstIterType CE) {
  ...
  auto Ret = ::mergeFunctionCall(Data, CalledObj, CE);
  if (Ret) {
...
ASTInformationExtractor::ASTInformationExtractor(const clang::ASTContext* CTX, metacg::Callgraph* CG,
                                                 bool CaptureCtorsDtors, bool CaptureStackCtorsDtors)
    : CTX(CTX), callgraph(CG), CaptureCtorsDtors(CaptureCtorsDtors), CaptureStackCtorsDtors(CaptureStackCtorsDtors) {}
...
void ASTInformationExtractor::addCallToCallGraph(const StringType& CallingFunctionName,
  ...
  if (CalledFunction != FuncDeclMap.end()) {
    for (const auto& calledFunctionMangle : getMangledNames(CalledFunction->second)) {
      auto Called = callgraph->getOrInsertNode(calledFunctionMangle);
      Called->setHasBody(CalledFunction->second->hasBody());
      bool virtualness = CalledFunction->second->isVirtualAsWritten() ||
                         (llvm::isa<clang::CXXMethodDecl>(CalledFunction->second) &&
                          llvm::cast<clang::CXXMethodDecl>(CalledFunction->second)->isVirtual());
      Called->setIsVirtual(virtualness);
      for (const auto& callerFunctionMangle : getMangledNames(CallingFunction->second)) {
        auto Caller = callgraph->getOrInsertNode(callerFunctionMangle);
        Caller->setHasBody(CallingFunction->second->hasBody());
        virtualness = CallingFunction->second->isVirtualAsWritten() ||
                      (llvm::isa<clang::CXXMethodDecl>(CallingFunction->second) &&
                       llvm::cast<clang::CXXMethodDecl>(CallingFunction->second)->isVirtual());
        Called->setIsVirtual(virtualness);
        callgraph->addEdge(Caller, Called);
      }
    }
  } else {
    assert(false);
...
void ASTInformationExtractor::handleDirectCalls() {
  for (const auto& Call : DirectCalls) {
    ...
    auto Ret = mergeFunctionCallImpl(Data, CalledUSR, CallerUSR, CallInfo, CallExprUSR);
    if (Ret) {
      addCallToCallGraph(Ret->second.first, Ret->second.second);
      for (const auto& ToMerge : Ret->first) {
        mergeFunctionCall(ToMerge.first, ToMerge.second);
...
FunctionInfo::FunctionInfo(clang::FunctionDecl* FD, clang::CXXMethodDecl* RealLambdaMethod) {
  ...
  MangledNames = getMangledNames(FD);
...
void calculateAliasInfo(clang::TranslationUnitDecl* TD, metacg::Callgraph* CG, nlohmann::json& J,
                        bool captureCtorsDtors, bool captureStackCtorsDtors) {
  assert(TD);
  assert(CG);
  ASTInformationExtractor Visitor(&TD->getASTContext(), CG, captureCtorsDtors, captureStackCtorsDtors);
