...
NodeDiff createNodeDiff(const NodeSummary& nsA, const NodeSummary& nsB, ComparisonMode mode) {
  ...
  if (!hasFlag(mode, ignoreEdges)) {
    ...
    bool edgeMetadataDifferent = !hasFlag(mode, ignoreEdgeMetadata) && nsA.edgeMetadata != nsB.edgeMetadata;
    ...
    for (const auto& callee : allCallees) {
      ...
      if (edgeDiff.onlyInA || edgeDiff.onlyInB || !edgeDiff.metadataOnlyInA.empty() ||
          !edgeDiff.metadataOnlyInB.empty()) {
        diff.edgeDiffs.push_back(std::move(edgeDiff));
      }
    }
    bool hasStructuralEdgeDiff = std::any_of(diff.edgeDiffs.begin(), diff.edgeDiffs.end(),
                                             [](const NodeDiff::EdgeDiff& ed) { return ed.onlyInA || ed.onlyInB; });
    bool hasEdgeMetadataDiff = std::any_of(
        diff.edgeDiffs.begin(), diff.edgeDiffs.end(),
        [](const NodeDiff::EdgeDiff& ed) { return !ed.metadataOnlyInA.empty() || !ed.metadataOnlyInB.empty(); });
