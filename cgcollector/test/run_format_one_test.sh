...
# Holds definitions for default programs and test functions
. ./testBase.sh
...
stat ../../${build_dir}/cgcollector/test/cgsimpletester >>log/testrun.log 2>&1
if [ $? -eq 1 ]; then
  echo "The file seems also non-present in ../${build_dir}/test. Aborting test. Failure! Please build the tester first."
  exit 1
else
  testerExe=../../${build_dir}/cgcollector/test/cgsimpletester
fi
...
for tc in ${testGlob}; do
  ...
  applyFileFormatOneToSingleTU ${tc}
  fail=$?
  fails=$((fails + fail))
done
...
for tc in ${testGlob}; do
  echo "Running test ${tc}"
  applyFileFormatOneToSingleTU ${tc} "--capture-ctors-dtors"
  fail=$?
  fails=$((fails + fail))
done
echo "Single file test failures: $fails"
...
for tc in "${multiTests[@]}"; do
  echo "Running test ${tc}"
  # Input files
  applyFileFormatOneToMultiTU ${tc}
  fail=$?
  fails=$((fails + fail))
done
echo "Multi file test failures: $fails"

echo -e "$fails failures occurred when running tests"
exit $fails
