...
public:
    ...
    UniquePtrValueIterator& operator++() { ++iter; return *this; }
...
class WrapperIterator {
    public:
        ...
        WrapperIterator& operator++() { ++iter; return *this; }
...
inline auto attachGraphPointerToNodes(const CgNodeRawPtrUSet nodes, const Callgraph& graph) {
    ...
    std::unordered_set<CgNodeWrapper, decltype(hash), decltype(equal)> newSet(
        WrapperIterator(nodes.begin(), graph),
        WrapperIterator(nodes.end(), graph),
        nodes.size(),
        hash, 
        equal
    );
